...
function buildWidgetRegistry(
  ...
  const lines: string[] = [
    "// Auto-generated by scripts/sync-chapters.ts — do not edit.",
    "",
    "export const chapterWidgetUrls: Record<string, string> = {",
  ];

  for (const { key, widgets } of entries) {
    const [subject, slug] = key.split("/");
    for (const widget of widgets) {
      const importPath = `/js/chapter-widgets/${subject}/${slug}/${widget}.js`;
      lines.push(`  "${key}/${widget}": "${importPath}",`);
    }
  }
...
async function main() {
  ...
  await writeIfChanged(GENERATED_META, `${JSON.stringify(metaFile, null, 2)}\n`);
  await writeIfChanged(GENERATED_WIDGETS, buildWidgetRegistry(widgetEntries));

  const chapterNav = await buildChapterNav(chapters);
  await writeIfChanged(GENERATED_NAV, `${JSON.stringify(chapterNav, null, 2)}\n`);
  ...
  await writeIfChanged(GENERATED_SUBJECTS_META, `${JSON.stringify(subjectsMeta, null, 2)}\n`);
  await writeIfChanged(GENERATED_SUBJECT_MODULES, buildSubjectModuleRegistry(subjectEntries));
